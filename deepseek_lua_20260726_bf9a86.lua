...
-- FIXED: Groundbulb & Vine ESP now properly ignores players and HumanoidRootParts
...
-- ============================================================
-- GROUNDBULB & VINE ESP - COMPLETE WORKING FIX
-- ============================================================
...
-- Helper: Check if a model is a player model (to ignore)
local function gvIsPlayerModel(model)
    if not model then return true end
    
    -- Check if it has a Humanoid (players have this)
    if model:FindFirstChildOfClass("Humanoid") then
        return true
    end
    
    -- Check if it's in the Players folder
    local playersFolder = svc.WS:FindFirstChild("Players")
    if playersFolder and model:IsDescendantOf(playersFolder) then
        return true
    end
    
    -- Check if it has a HumanoidRootPart (players have this)
    if model:FindFirstChild("HumanoidRootPart") then
        return true
    end
    
    -- Check if it's a descendant of any player's character
...
-- Get the top part of a model for label positioning
local function gvGetTopPart(model)
    ...
    for _, descendant in ipairs(model:GetDescendants()) do
        if descendant:IsA("BasePart") then
            -- Ignore HumanoidRootPart - it's a player part
    ...
    if not highestPart then
        for _, descendant in ipairs(model:GetDescendants()) do
            if descendant:IsA("BasePart") and descendant.Name ~= "Handle" and descendant.Name ~= "HumanoidRootPart" then
                highestPart = descendant
                break
            end
        end
...
-- Check if model is Groundbulb
local function gvIsGroundbulb(model)
    if not model or not model:IsA("Model") then return false end
    if gvIsPlayerModel(model) then return false end
    
    local name = model.Name:lower()
    if name:find("groundbulb") or name:find("bulb") then
        return true
    end
    ...
    for _, desc in ipairs(model:GetDescendants()) do
        if desc:IsA("BasePart") then
            ...
            if dname:find("flower") or dname:find("nectar") or dname:find("root") then
                return true
            end
        end
    end
...
-- Check if model is Vine
local function gvIsVine(model)
    if not model or not model:IsA("Model") then return false end
    if gvIsPlayerModel(model) then return false end
    
    local name = model.Name:lower()
    if name:find("vine") then
        return true
    end
    ...
    for _, desc in ipairs(model:GetDescendants()) do
        if desc:IsA("BasePart") then
            local dname = desc.Name:lower()
            if dname:find("tentacle") or dname:find("vine") then
                return true
            end
        end
    end
...
-- Highlight a single entity
local function gvHighlightEntity(model, displayName)
    if not model or not model:IsA("Model") then return end
    if gvEsp.trackedEntities[model] then return end
    if gvIsPlayerModel(model) then return end
    ...
    highlight.FillColor = gvEsp.highlightColor
    highlight.FillTransparency = gvEsp.fillTransparency
    highlight.OutlineColor = gvEsp.highlightColor
    highlight.OutlineTransparency = gvEsp.outlineTransparency
    ...
    billboard.MaxDistance = gvEsp.maxDistance
...
-- Evaluate and highlight a model
local function gvEvaluateAndHighlight(child)
    if not child:IsA("Model") then return end
    if gvIsPlayerModel(child) then return end
    
    local isGroundbulb = gvIsGroundbulb(child)
    local isVine = gvIsVine(child)
...
local function gvStart()
    ...
    gvEsp.scanThread = task.spawn(function()
        while gvEsp.on do
            pcall(gvUpdateLoop)
            task.wait(gvEsp.scanInterval)
        end
    end)
    
    -- Setup listener for new models
...
local function gvStop()
    ...
    if gvEsp.scanThread then
        task.cancel(gvEsp.scanThread)
...
-- GROUNDBULB & VINE SETTINGS SECTION
...
secGVEsp:Slider({
    ...
    Flag = "gvTransparency",
    Step = 0.05,
    Value = { Min = 0.1, Max = 1.0, Default = gvEsp.fillTransparency },
    Callback = function(v)
        pcall(function()
            gvEsp.fillTransparency = v
