...
local function gvStartStandalone()
    ...
    gvStandaloneThread = task.spawn(function()
        ...
        local function highlightEntity(model, displayName)
            ...
            -- Clean up any existing highlights on this model
            local existingHl = model:FindFirstChild("EntityHighlight")
            if existingHl then existingHl:Destroy() end
            local existingBb = model:FindFirstChild("EntityTag")
        ...
        local function evaluateAndHighlight(child)
            ...
            -- Filter: Don't highlight static map objects, only entity models
            -- Only process models that are direct children of "Ingame" > "Map"
            if child.Parent and child.Parent.Name == "Map" then
                local grandparent = child.Parent.Parent
                if not grandparent or grandparent.Name ~= "Ingame" then
                    return  -- Skip map structure objects
                end
            else
                return  -- Only process direct children of the Map folder in Ingame
            end
        ...
        -- Use Stepped instead of Heartbeat for better performance with ESP
...
local function gvStopStandalone()
    ...
    for _, obj in ipairs(gvStandaloneObjects) do
        ...
        pcall(function()
            ...
            -- Also clean up any leftover highlights on the model
            if model then
                local oldHl = model:FindFirstChild("EntityHighlight")
                if oldHl then oldHl:Destroy() end
                local oldBb = model:FindFirstChild("EntityTag")
...
svc.Run.RenderStepped:Connect(function()
    ...
    if guestCfg.aimPunchActive and g.humanoid and g.hrp and g.punchAiming then
        ...
        if elapsed > guestCfg.aimPunchDuration then
            g.punchAiming = false
            -- ALWAYS restore AutoRotate when punch expires (FIXED: was only restoring if g.originalWS existed)
            if g.originalAutoRotate ~= nil then
                g.humanoid.AutoRotate = g.originalAutoRotate
            end
            -- Restore movement stats if they were saved
            if g.originalWS then
                g.humanoid.WalkSpeed = g.originalWS
                g.humanoid.JumpPower = g.originalJP
                g.originalWS = nil; g.originalJP = nil
            end
            g.originalAutoRotate = nil
        else
...
local function hdt_execute(killer, killerHRP)
    ...
    -- Create BodyVelocity for smooth pulling
